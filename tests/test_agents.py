...
class TestAgents:
    ...
    def test_vyakaran_rakshak_typos(self):
        ...
        assert any(i['type'] == 'typo' and 'છાપો' in i['message'] for i in issues)
    ...
    def test_bandharan_rakshak_mixed_language(self):
        ...
        """
        issues = agent.check_file(bad_code)
        assert any(i['type'] == 'mixed_language' for i in issues)
...
class TestAdvancedAgents:
    ...
    def test_suraksha_eval(self):
        ...
        bad_code = "eval('rm -rf')"
        issues = agent.check_file(bad_code)
        assert any(i['type'] == 'security_danger' for i in issues)

    def test_suraksha_secret(self):
        agent = SurakshaRakshak()
        bad_code = "password = '123'"
        issues = agent.check_file(bad_code)
        assert any(i['type'] == 'security_secret' for i in issues)
    ...
    def test_karyakshamata_import(self):
        ...
        """
        issues = agent.check_file(bad_code)
        assert any(i['type'] == 'perf_import' for i in issues)
    ...
    def test_jatilata_length(self):
        ...
        issues = agent.check_file(bad_code)
        assert any(i['type'] == 'complexity_length' for i in issues)
